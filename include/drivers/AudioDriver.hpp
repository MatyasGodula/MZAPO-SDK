...
struct Tone {
    public:
        constexpr Tone(uint32_t frequency, uint32_t duty) : frequency(frequency), duty(duty) {}

        constexpr uint32_t get_frequency() { return frequency; }

        constexpr uint32_t get_duty() { return duty; }
